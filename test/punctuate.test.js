...
test('Can punctuate text', async () => {
    ...
    for (const example of examples) {
        const punctuated = await punctuateText( example.raw )
        ...
        for ( let i = 0; i < punctuated.length; i += segmentLength ) {
            ...
            const expected = example.expectedPunctuation.substring( i, i + segmentLength )
